...
const Tasks = () => {
    ...
    const updateTask = (id, newTitle, newTag, newDate, completed = null, newPriority = null, pinned = null) => {
        setTasks(tasks.map(task => {
            if (task.id === id) {
                const updatedTask = {
                    ...
                    title: newTitle,
                    tag: newTag,
                    dueDate: newDate ? new Date(newDate + "T00:00:00").toISOString() : task.dueDate,
                };
    ...
    const getFilterTitle = () => {
        switch (filter) {
            case "day": return "Tâches du jour";
            case "week": return "Tâches de la semaine";
            case "month": return "Tâches du mois";
            case "completed": return "Tâches complétées";
            default: return "";
        }
    };
    ...
    const filteredTasks = tasks.filter(task => {
        ...
        const matchTag = tagSearch.trim() === "" || task.tag?.toLowerCase().includes(tagSearch.toLowerCase());
    ...
    <div style={{ display: "flex", gap: "10px" }}>
        <button className="delete-visible-btn" onClick={() => selectAllVisibleTasks(filteredTasks)}>Sélectionner</button>
        <button className="delete-visible-btn" onClick={deleteSelectedTasks} disabled={selectedIds.length === 0}>🗑️</button>
        <button className="delete-visible-btn" onClick={completeSelectedTasks} disabled={selectedIds.length === 0}>✅</button>
    </div>
) : (
    <div style={{ display: "flex", gap: "10px" }}>
        <button className="delete-visible-btn" onClick={() => selectAllVisibleTasks(tasks.filter(task => task.completed))}>Sélectionner</button>
        <button className="delete-visible-btn" onClick={restoreSelectedTasks} disabled={selectedIds.length === 0}>🔁</button>
        <button className="delete-visible-btn" onClick={deleteSelectedTasks} disabled={selectedIds.length === 0}>🗑️</button>
    </div>
)}
